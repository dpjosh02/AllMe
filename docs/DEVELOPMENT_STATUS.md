...
- Calendar v1 is a local-cache planning surface backed by Google Calendar OAuth/sync. `/calendar` includes sync action/status, source visibility filters, a next-seven-days planner, next-events list, day agenda drawer, event detail drawer, and local AllMe review states (`Needs prep`, `Done`, `Ignored`, `Unreviewed`). Today consumes cached calendar rows only.
- Calendar Phase 2 adds controlled provider writes behind explicit user actions. Implemented slices include event-linked notes, write-scope reauthorization, publish-note-to-description, non-recurring create/edit/delete, and this-event-only recurring occurrence update support. Recurring writes require explicit `Only this event` selection; `This and following` and `Entire series` remain deferred.
- Calendar Phase 2.9D smoke status: `/calendar` loads after the recurring-edit UI slice and visible non-recurring flows still render. The current local cache contains two recurring birthday occurrences, but they are outside the visible planning window and are missing `original_start_at`, so they are not safe real-provider candidates for this-event-only write smoke. A real upcoming Google recurring occurrence with `original_start_at` should be used before advancing beyond recurrence UI guardrails.
...
- `PageHero` and `PageSection` keep headers compact by rendering title/action/icon context without eyebrow labels or descriptive header subtext unless a future workflow requires a dedicated exception.
